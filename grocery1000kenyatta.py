...
class NewForm(tk.Toplevel):
    ...
    def __init__(self):
        ...
        for label in self.labels.keys():
            K1 = tk.Frame(self, relief=tk.RAISED, bd=2)
            K1.pack(side=tk.TOP, expand=tk.YES, fill=tk.X)
            tk.Label(K1, text=label).pack(side=tk.LEFT)
            ent = tk.Entry(K1, textvar=self.variables[label])
    ...
    def Submit(self):
        for variable_value in self.variables.values():
            if len(variable_value.get()) == 0:
                tk.messagebox.showinfo('ARE YOU SLOW?', 'Actually enter some values, please.')
                return
        self.destroy()
...
class Main(tk.Frame):
    ...
    def __init__(self, parent=None):
        tk.Frame.__init__(self, parent, relief=tk.RIDGE, bd=4, width=200, height=350)
        self.master.title('Grocery List')
        self.pack()
        parent.minsize(width=500, height=300)
        parent.maxsize(width=700, height=400)
        self.add_menu()
        self.add_info()
        self.add_buttons()
    ...
    def add_info(self):
        ...
        self.sbar.config(command=self.lbox)
    ...
    def add_buttons(self):
        ...
        tk.Button(frame, text='Add grocery item', command=self.add_item).pack(side=tk.LEFT, expand=tk.YES)
        tk.Button(frame, text='Delete grocery item', command=self.delete_item).pack(side=tk.LEFT, expand=tk.YES)
    ...
    def update_list(self):
        self.lbox.insert(0, self.popup.variables[' Grocery Item'].get() + ', ' + self.popup.variables['Price'].get() + '\n')
